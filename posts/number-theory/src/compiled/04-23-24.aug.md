...
## tricky part of the argument:

Define $L(s,\chi) = \sum_{n\ge 1} \chi(n)/n^{s}$.

1. **$L(s, \chi)$ is analytic on $\Re(s)>0$ for $\chi\neq
   \chi_0$** (the convergence is clear; presumably the
   analytic-ness just happens because it's a reasonable function).
2. $L(s,\chi_0)-\frac{1}{s-1}$ extends analytically to a function
   on $\Re(s>0)$ with a single pole, namely at $s=1$. (again, the
   convergence is clear, but the analytic-ness, my complex
   analysis is a bit weak.)
